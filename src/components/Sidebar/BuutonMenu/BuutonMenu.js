...
const BuutonMenu = ({dopMenu, setDopMenu})=>{
    ...
    const DopMenu = (clickVariant) => {
      ...
      if (clickVariant === 1) {
        setDopMenu({menu: Menu[currentIndex + clickVariant] || Menu[0], menuMovement: clickVariant});
      } else if (clickVariant === -1) {
        setDopMenu({menu: Menu[currentIndex + clickVariant] || Menu[Menu.length - 1], menuMovement: clickVariant});
      }
    };
